...
def set_hugepages(path, reqpages):
    ...
    try:
        ...
        sys.exit(
            "Invalid page size. Valid page sizes: {}".format(get_valid_page_sizes(path))
        )
    gotpages = get_hugepages(path)
    if gotpages != reqpages:
        sys.exit(
            "Unable to set pages ({} instead of {} in {}).".format(
    ...
    for numa_path in glob.glob("/sys/devices/system/node/node*"):
        ...
        for hdir in os.listdir(path):
            ...
            if pages > 0:
                ...
                print(
                    "{:<4} {:<5} {:<6} {}".format(
                        node, pages, fmt_memsize(kb), fmt_memsize(pages * kb)
                    )
                )
...
def show_non_numa_pages():
    ...
    for hdir in os.listdir(path):
        ...
        if pages > 0:
            kb = int(hdir[10:-2])
            print(
                "{:<5} {:<6} {}".format(pages, fmt_memsize(kb), fmt_memsize(pages * kb))
            )
...
def clear_pages():
    ...
    if is_numa():
        dirs = glob.glob("/sys/devices/system/node/node*/hugepages/hugepages-*")
    else:
...
""",
    ...
    parser.add_argument(
        "--mount", "-m", action="store_true", help="mount the huge page filesystem"
    )
    parser.add_argument(
        "--unmount",
        "-u",
    ...
    parser.add_argument(
        "--directory", "-d", metavar="DIR", default=HUGE_MOUNT, help="mount point"
    )
    parser.add_argument(
        "--user", "-U", metavar="UID", help="set the mounted directory owner user"
    )
    parser.add_argument(
        "--group", "-G", metavar="GID", help="set the mounted directory owner group"
    )
    parser.add_argument("--node", "-n", help="select numa node to reserve pages on")
    parser.add_argument(
    ...
    if args.reserve:
        ...
        if reserve_kb % pagesize_kb != 0:
            sys.exit(
                "Huge reservation {}kB is not a multiple of page size {}kB".format(
                    reserve_kb, pagesize_kb
                )
            )
        reserve_pages(int(reserve_kb / pagesize_kb), pagesize_kb, node=args.node)
    if args.mount:
...
def get_hugepage_status():
    """Get huge page status"""
    pagesize = default_pagesize()
    mountpoints = get_mountpoints()
    return {
        "default_pagesize_kb": pagesize,
