...
def query(cmd, file_prefix=None, instance=None):
    """
    Single command query.
    """
    results = query_batch([cmd], file_prefix=file_prefix, instance=instance)
    return results[cmd]


def query_batch(cmds, file_prefix=None, instance=None):
    """
    Multiple commands in a single connection.
    Returns dict keyed by command string.
    """
    ...
    if instance > 0:
        sock_path += ":{}".format(instance)

    sock = socket.socket(socket.AF_UNIX, socket.SOCK_SEQPACKET)
    try:
        sock.connect(sock_path)
        handshake = read_socket(sock, 1024, echo=False)
        buf_len = handshake["max_output_len"]

        results = {}
        for cmd in cmds:
            sock.send(cmd.encode())
            results[cmd] = read_socket(sock, buf_len, echo=False)
    finally:
        sock.close()
